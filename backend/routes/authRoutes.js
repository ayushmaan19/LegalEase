...
router.post("/register", async (req, res) => {
  try {
    ...
    if (role === "lawyer" && !enrollmentNumber)
      return res
        .status(400)
        .json({ msg: "Enrollment number is required for lawyers" });
    ...
    if (role === "lawyer" && (await User.findOne({ enrollmentNumber })))
  ...
  try {
    ...
    if (!user.password) {
      return res
        .status(400)
        .json({
          msg: 'You signed up using Google. Please use "Continue with Google" to log in.',
        });
    }
  ...
  try {
    ...
    if (user) {
      ...
      if (user) {
        ...
        while (userExists) {
          tempUsername = `${tempUsername}${Math.floor(
            100 + Math.random() * 900
          )}`;
